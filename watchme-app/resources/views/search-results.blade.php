...
<section class="section">
    ...
    <div class="pagination">
        @if($currentPage > 1)
            <a href="{{ route('movies.search', ['query' => $query, 'page' => $currentPage - 1]) }}" class="page-btn">&laquo;</a>
        @else
        ...
        @if($rangeEnd < $totalPages)
            @if($rangeEnd < $totalPages - 1)
                <span class="page-btn disabled">...</span>
            @endif
            <a href="{{ route('movies.search', ['query' => $query, 'page' => $totalPages]) }}" class="page-btn">{{ $totalPages }}</a>
        @endif

        @if($currentPage < $totalPages)
            <a href="{{ route('movies.search', ['query' => $query, 'page' => $currentPage + 1]) }}" class="page-btn">&raquo;</a>
        @else
